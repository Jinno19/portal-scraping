...
async function contextGeter(title, instructor, lectureLength, page, number) {
    try {
        for (let i = number+1; i <= lectureLength; i++) {
            ...
            esReference = refTitle.toString().replace(/[\f\r\t\v]/g, '');
            startNumber++;
...
export async function goToEsE6Syllabus() {
    ...
    await page.select('table > tbody > tr:nth-child(2) > td:nth-child(2) > #jikanwariShozokuCode', 'ESE6');
