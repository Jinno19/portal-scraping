...
async function contextGeter(title, instructor, lectureLength, page, number) {
    try {
        for (let i = number+1; i <= lectureLength; i++) {
            ...
            x1Reference = refTitle.toString().replace(/[\f\r\t\v]/g, '');
            startNumber++;
            ...
            [
                {
                    ...
                    // eslint-disable-next-line
                    "Reference": x1Reference
                // eslint-disable-next-line
                }
            ]);
        console.log(title);
        console.log(instructor);
        console.log(x1Reference);
...
export async function goToX1Syllabus() {
    ...
    await page.select('table > tbody > tr:nth-child(2) > td:nth-child(2) > #jikanwariShozokuCode', 'X1');
