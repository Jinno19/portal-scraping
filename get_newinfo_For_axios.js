...
const ACCOUNT_PASS = process.env.PASSWORD;
...
async function getPage(uri) {
    ...
    for (let s = 0; s < page_uri.length; s++) {
        ...
        $2('.post > .entry-body > p').each((_n, elem) => {
            context_fragment.push($2(elem).text().replace(/[\f\r\n\t\v]/g, ''));
        });
        const entryBody = context_fragment.filter(v => v.length > 2).join('');
        context.push(entryBody);
        const tag_list = $(`#tab2 > .front_news_list > li:nth-child(${s+1}) > span`)
            .toArray()
            .map(elm => $(elm).text());
